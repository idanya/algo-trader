...
def generate_example_templates():
    ...
    save_pipeline_spec(
        "backtest_history_buckets_backtester.json",
        BacktestPipelines.build_mongodb_history_buckets_backtester(f"{EXAMPLE_TEMPLATES_DIR}/bins.json"),
    )

    save_pipeline_spec(
        "backtest_technicals_with_buckets_calculator.json",
        LoadersPipelines.build_technicals_with_buckets_calculator(
            f"{EXAMPLE_TEMPLATES_DIR}/bins.json", BIN_COUNT, f"{EXAMPLE_TEMPLATES_DIR}/correlation.json"
        ),
    )
    ...
    save_pipeline_spec(
        "loader_technicals_with_buckets_matcher.json",
        LoadersPipelines.build_technicals_with_buckets_matcher(
            f"{EXAMPLE_TEMPLATES_DIR}/bins.json", f"{EXAMPLE_TEMPLATES_DIR}/correlation.json"
        ),
    )

    save_pipeline_spec(
        "backtest_history_similarity_backtester.json",
        BacktestPipelines.build_mongodb_history_similarity_backtester(f"{EXAMPLE_TEMPLATES_DIR}/bins.json"),
    )
