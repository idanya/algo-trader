...
class TestAssetCorrelationProcessor(TestCase):
    def setUp(self) -> None:
        super().setUp()
        x = [
            generate_candle_with_price_and_symbol(
                "X", TimeSpan.Day, datetime.now() - timedelta(days=c), c + random.randint(1, 10)
            )
            for c in range(1, 49)
        ]
        y = [
            generate_candle_with_price_and_symbol(
                "Y", TimeSpan.Day, datetime.now() - timedelta(days=c), c + random.randint(1, 10)
            )
            for c in range(1, 49)
        ]
        z = [
            generate_candle_with_price_and_symbol(
                "Z", TimeSpan.Day, datetime.now() - timedelta(days=c), c + random.randint(1, 10)
            )
            for c in range(1, 49)
        ]
    ...
    def test_correlation(self):
        ...
        correlations_file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "../configs/correlations.json"
        )
